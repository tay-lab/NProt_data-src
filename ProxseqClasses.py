...
Author: Junjie Xia latest_version
...
class plaObject:
    ...
    def calculateProteinPairCount(self):
        '''
        Combine probeA:probeB and probeB:probeA
        '''
        # Get AB1 and AB2 of each row of data
        AB1 = np.array([s.split(self.sep)[0] for s in self.pla_count.index])
        AB2 = np.array([s.split(self.sep)[1] for s in self.pla_count.index])
        ...
        new_index = []
        for i in np.arange(len(A_unique)):
            for j in range(i, len(B_unique)):
                new_index.append(f'{A_unique[i]}:{B_unique[j]}')
